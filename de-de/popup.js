...
// Ressourcentyp-Definitionen nach CAF-Abkürzungen
...
function loadConfig(callback) {
  chrome.storage.sync.get(["azureNamingConfig"], (data) => {
    ...
    // Backwards-Compatibility: alte Felder company/workload mappen
    cfg.env = cfg.env || "dev";
    ...
    // auch alte Felder konsistent halten
...
function getPatternForType(typeKey, cfg) {
  ...
  // Default-Pattern jetzt mit Location & Application
...
function applyPattern(pattern, cfg, resourceTypeKey) {
  ...
  // Neu: Location / Application, aber alte Platzhalter weiter unterstützen
  const loc = cfg.location || cfg.company || "loc";
  const app = cfg.application || cfg.workload || "app";

  name = name.replace(/{location}/gi, loc);
  name = name.replace(/{company}/gi, loc); // Backwards-Kompatibilität

  name = name.replace(/{application}/gi, app);
  name = name.replace(/{workload}/gi, app); // Backwards-Kompatibilität
  ...
  saveBtn.addEventListener("click", () => {
    loadConfig((cfg) => {
      ...
      saveConfig(cfg, () => {
        ...
        setTimeout(() => (saveBtn.textContent = "Konfiguration speichern"), 1200);
      });
    });
  });
